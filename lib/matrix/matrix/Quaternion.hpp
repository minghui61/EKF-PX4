 ...
 *
 * 所有旋转和坐标系遵循右手定则。
 * 采用 Hamilton 四元数约定（包括其乘积定义）。
 *
 * 要将帧 b 中的向量 v_b 按由四元数 q_nb（表示从帧 b 到帧 n 的旋转）定义的右手旋转变换到帧 n，
 * 可使用以下操作：
 * v_n = q_nb * [0; v_b] * q_nb^(-1)
 *
 * 类似于 DCM： v_n = C_nb * v_b（向量旋转）
 * M_n = C_nb * M_b * C_nb^(-1)（矩阵旋转）
 *
 * 或者相反的操作：
 * v_b = q_nb^(-1) * [0; v_n] * q_nb
 *
 * 其中 q_nb^(-1) 表示四元数 q_nb 的逆（即 q_bn）。
 * 两个四元数的乘积 z = q2 * q1 表示先执行 q1 后执行 q2 的本征旋转。
 * 四元数的第一个分量为实部，因此表示零旋转的四元数定义为 (1,0,0,0)。
 *
