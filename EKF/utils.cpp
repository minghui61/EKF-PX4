...
{
		// 根据 312 顺序的 Tait-Bryan 旋转计算 frame2 到 frame1 的旋转矩阵
		const float c2 = cosf(rot312(2)); // 第三次旋转为俯仰
		const float s2 = sinf(rot312(2));
		const float s1 = sinf(rot312(1)); // 第二次旋转为滚转
		const float c1 = cosf(rot312(1));
		const float s0 = sinf(rot312(0)); // 第一次旋转为偏航
...
float getEuler321Yaw(const matrix::Quatf& q) {
	// 取自 yaw_input_321.c 文件中的数值，生成代码来自：
...
float getEuler312Yaw(const matrix::Quatf& q) {
	// 取自 yaw_input_312.c 文件中的数值，生成代码来自：
